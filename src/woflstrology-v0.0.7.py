...
def generate_natal_chart_reading(natal_positions, house_data, horoscope_db):
    ...
    if aspects:
        ...
        for aspect in aspects_sorted[:10]:  # Show top 10 most significant
            planet1 = aspect["planet1"]
            planet2 = aspect["planet2"]
            aspect_type = aspect["type"]
            angle = aspect["angle"]
            
            # Create the key for looking up interpretation
            # Sort planets alphabetically for consistent key
            planets_sorted = tuple(sorted([planet1, planet2]))
            aspect_key = f"{planets_sorted[0]}_{planets_sorted[1]}"
            
            # Get detailed interpretation
            detailed_interp = horoscope_db.get("natal_chart", {}).get("aspect_interpretations", {}).get(
                aspect_key, {}
            ).get(aspect_type, f"These planetary energies interact through {aspect_type}.")
            
            reading += f"**{planet1} {aspect_type.title()} {planet2}** ({angle:.1f}°)\n"
            reading += f"{detailed_interp}\n\n"
        
        if len(aspects) > 10:
            reading += f"*Your chart contains {len(aspects) - 10} additional aspects forming the complete pattern of your psyche.*\n\n"
