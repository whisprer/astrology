...
def calculate_asteroid(asteroid_number, year, month, day, hour, minute, second, timezone_str="UTC"):
    ...
    Returns None if asteroid cannot be calculated
    """
    try:
        tz = pytz.timezone(timezone_str)
        local_dt = tz.localize(datetime(year, month, day, hour, minute, second))
        ...
        print(f"    [DEBUG] Attempting asteroid #{asteroid_number} (SE num: {se_asteroid_num}) using astorb.dat")
        
        # Use SWIEPH flag which should use files in ephe_path (including astorb.dat)
        # The key is that astorb.dat MUST be in your ephe_path directory
        result, ret_flag = swe.calc_ut(jd, se_asteroid_num, swe.FLG_SWIEPH)
        
        print(f"    [DEBUG] SUCCESS! Got result: {result[0]:.2f}°")
