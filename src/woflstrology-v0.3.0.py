...
def test_astorb_access():
    """Test if astorb.dat is accessible"""
    import os
    
    print("\n🔍 DEBUGGING ASTORB.DAT ACCESS...")
    print(f"Ephemeris path variable: {ephe_path}")
    
    astorb_path = os.path.join(ephe_path, 'astorb.dat')
    print(f"Looking for: {astorb_path}")
    print(f"File exists: {os.path.exists(astorb_path)}")
    
    if os.path.exists(astorb_path):
        file_size = os.path.getsize(astorb_path)
        print(f"File size: {file_size / (1024*1024):.1f} MB")
        
        # Read first few lines
        with open(astorb_path, 'r', encoding='latin-1') as f:
            print("\nFirst 3 lines of astorb.dat:")
            for i in range(3):
                line = f.readline()
                print(f"  {line.strip()}")
    
    # Test if path is actually being used by trying to calculate something
    print("\n🧪 Testing Swiss Ephemeris calculation...")
    print(f"Attempting to use path: {ephe_path}")
    
    # Try setting it again right here
    swe.set_ephe_path(ephe_path)
    print(f"Path set again to: {ephe_path}")
    
    # Try to calculate Ceres (asteroid 1)
    try:
        # FIXED: Use decimal hour (4 arguments only!)
        jd = swe.julday(2025, 11, 27, 12.0)  # year, month, day, decimal_hour
        result, ret = swe.calc_ut(jd, swe.AST_OFFSET + 1, 0)  # Ceres
        print(f"✓ Ceres calculation SUCCESS! Position: {result[0]:.2f}°")
    except Exception as e:
        print(f"✗ Ceres calculation FAILED: {e}")
    
    print()

def calculate_asteroid(asteroid_number, year, month, day, hour, minute, second, timezone_str="UTC"):
    """
    Calculate position of a specific asteroid by number using astorb.dat
    """
    print(f"  → Attempting asteroid #{asteroid_number}...")
    ...
    try:
        ...
        # Swiss Ephemeris asteroid numbering: number + 10000
        se_asteroid_num = swe.AST_OFFSET + asteroid_number
        
        print(f"    JD: {jd}, SE number: {se_asteroid_num}")
        
        # Try with JUST position calculation - NO speed flag at all!
        try:
            print(f"    Trying FLG_SWIEPH...")
            result, ret_flag = swe.calc_ut(jd, se_asteroid_num, swe.FLG_SWIEPH)
            print(f"    SUCCESS with FLG_SWIEPH!")
        except Exception as e1:
            print(f"    FLG_SWIEPH failed: {e1}")
            print(f"    Trying flag 0...")
            result, ret_flag = swe.calc_ut(jd, se_asteroid_num, 0)
            print(f"    SUCCESS with flag 0!")
        
        longitude = result[0]
        
        # Speed might be in result[3] if available, otherwise 0
        try:
            speed = result[3]
        except:
            speed = 0.0
        ...
        print(f"    ✓ Asteroid #{asteroid_number}: {sign} {longitude % 30:.1f}°\n")
        ...
        return {
            ...
            "degrees_in_sign": longitude % 30,
            "speed": speed,
            "retrograde": speed < 0 if speed != 0 else False
        }
    except Exception as e:
        print(f"    ✗ FAILED: {e}\n")
        return None
...
def main():
    ...
    if do_major_asteroids == 'y':
        ...
        # ADD THIS DEBUG:
        print(f"\n🔍 DEBUG major_asteroids dictionary:")
        print(f"Type: {type(major_asteroids)}")
        print(f"Length: {len(major_asteroids)}")
        print(f"Keys: {major_asteroids.keys() if major_asteroids else 'EMPTY!'}")
        print(f"Contents: {major_asteroids}\n")

        print(f"\n{'=' * 70}")
        print(f"MAJOR ASTEROIDS - The Goddess Energies")
        print(f"{'=' * 70}\n")
        ...
        print(f"\n🔍 DEBUG major_data:")
        print(f"major_data type: {type(major_data)}")
        print(f"major_data length: {len(major_data) if major_data else 'EMPTY or None'}")
        print(f"major_data keys: {list(major_data.keys()) if major_data else 'NO KEYS'}")
        print(f"major_data contents: {major_data}\n")

        # Also check the path to get there:
        print(f"horoscope_db has 'natal_chart': {'natal_chart' in horoscope_db}")
        natal_chart = horoscope_db.get("natal_chart", {})
        print(f"natal_chart has 'asteroids': {'asteroids' in natal_chart}")
        asteroids = natal_chart.get("asteroids", {})
        print(f"asteroids has 'major_asteroids': {'major_asteroids' in asteroids}")
        print(f"asteroids keys: {list(asteroids.keys()) if asteroids else 'NO KEYS'}\n")
        ...
        for ast_name, ast_position in major_asteroids.items():
            ...
            # Find asteroid data by name
            ast_info = None
            for num_key, data in major_data.items():
                print(f"Checking key {num_key}: {data.get('name', 'NO NAME')}")
                if data["name"] == ast_name:
                    ast_info = data
                    print(f"✓ Found match for {ast_name}!")
                    break
                
            if not ast_info:
                print(f"✗ No info found for {ast_name} in database!")
                continue  # Skip to next asteroid
    
            if ast_info:
                keywords = ast_info.get("keywords", "")
                interpretation = ast_info.get("interpretation", "")
                sign_interp = ast_info.get("in_signs", {}).get(sign, f"{ast_name} in {sign} adds unique energy.")
                
                print(f"**{ast_name} in {sign} {degree:.1f}°** (House {house}){retro}")
                print(f"*{keywords}*")
                print(f"{interpretation}")
                print(f"\n**In {sign}:** {sign_interp}\n")
    ...
    # ADD PERSONAL NAME ASTEROID FINDER
    if do_name_search == 'y' and search_names == 'y':
        print("\n⏳ Searching for name asteroids...")
        
        print(f"\n{'=' * 70}")
        print(f"PERSONAL NAME ASTEROIDS")
        print(f"{'=' * 70}\n")
        ...
        for search_name in search_names:
            ...
            if matches:
                ...
                for match in matches:
                    ast_name = match["name"]
                    ast_num = match["number"]
                    
                    # Calculate this asteroid
                    ast_pos = calculate_asteroid(
                        ast_num, natal_year, natal_month, natal_day,
                        natal_hour, natal_minute, 0, birth_tz
                    )
                    
                    if ast_pos:
                        sign = ast_pos["sign"]
                        degree = ast_pos["degrees_in_sign"]
                        house = find_house_for_planet(ast_pos["longitude"], house_data["cusps"])
                        retro = " ℞" if ast_pos["retrograde"] else ""
                        
                        print(f"✨ **Asteroid {ast_name} (#{ast_num})**")
                        print(f"   Located at {sign} {degree:.1f}° in House {house}{retro}")
                        
                        # Check for conjunctions to natal planets
                        conjunctions = []
                        for planet_name, planet_data in natal_chart_positions.items():
                            angle = calculate_aspect_angle(ast_pos["longitude"], planet_data["longitude"])
                            if angle <= 3:  # Within 3° conjunction
                                conjunctions.append((planet_name, angle))
                        
                        if conjunctions:
                            print(f"   🌟 **CONJUNCT:**")
                            for planet, orb in conjunctions:
                                print(f"      • {planet} (within {orb:.1f}°) - Your name energy merges with {planet}!")
                        
                        print()
            else:
                print(f"⚠ No asteroid found for '{search_name}' in database.")
                print(f"   (Note: Limited name database - many exist but aren't all loaded)\n")
        
        if not found_any:
            print("No matching asteroids found in the current database.\n")
