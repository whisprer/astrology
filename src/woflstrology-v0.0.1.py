#!/usr/bin/env python3
"""
Advanced Astrological Position Calculator with Natal Chart Houses
Calculates personalized interpretations based on birth chart and current transits
"""
...
def calculate_houses(year, month, day, hour, minute, second, lat, lon, timezone_str="UTC"):
    """
    Calculate house cusps using Placidus system [web:21][web:24]
    Returns list of house cusps (1-12) with their zodiac positions
    """
    # Convert to UTC
    ...
    # swe.houses returns (cusps, ascmc) where cusps[1-12] are house cusps
    cusps, ascmc = swe.houses(jd, lat, lon, b'P')  # 'P' = Placidus
    
    # ascmc contains: [0]=Ascendant, [1]=MC, [2]=ARMC, [3]=Vertex, etc.
    house_data = {
        "ascendant": {
            "longitude": ascmc[0],
...
def find_house_for_planet(planet_longitude, house_cusps):
    """
    Determine which house a planet is in based on its longitude [web:21]
...
def get_retrograde_interpretation_by_element(planet, element):
    """
    Get element-specific retrograde interpretation [web:13]
    """
    interpretations = {
        "Mercury": {
...
def get_retrograde_interpretation_by_house(planet, house_num):
    """
    Get house-specific retrograde interpretation [web:17][web:26]
    """
...
def check_natal_rulership(planet, natal_sun_sign):
    """
    Check if planet rules the natal sun sign [web:16][web:27]
    """
    ruled_signs = RULERSHIPS.get(planet, [])
    return natal_sun_sign in ruled_signs


def generate_personalized_reading(current_positions, house_data, natal_sun_sign, natal_moon_sign, birth_lat, birth_lon):
    """
...
def main():
    ...
    print("\n💡 Enter birth location (for house calculations)")
    print("You can find coordinates at: https://www.latlong.net/")
    birth_lat = float(input("Birth latitude (e.g., 51.5074 for London): "))
    birth_lon = float(input("Birth longitude (e.g., -0.1278 for London): "))
    birth_tz = input("Birth timezone (e.g., UTC, Europe/London, America/New_York): ").strip() or "UTC"
    
    print("\n⭐ What is your Sun sign? (e.g., Aquarius, Leo, Pisces)")
    natal_sun_sign = input("Sun sign: ").strip().title()
    
    print("🌙 What is your Moon sign?")
    natal_moon_sign = input("Moon sign: ").strip().title()
